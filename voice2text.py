#!/usr/bin/env python3
"""Voice-to-Text: press F13 to start/stop recording, transcribe with AI.
...
Set hotkey via VOICE2TEXT_HOTKEY env variable (default: f13)
Options: f1-f13, insert (insert only works on external keyboards)

Set model via: export VOICE2TEXT_MODEL="google/gemini-3-flash-preview"
"""
...
class Config:
    ...
    model_alias: str = "google/gemini-3-flash-preview"
    hotkey_name: str = "f13"
    ...
    transcribe_system_prompt: str = (
        "Transcribe audio to plain text. Output only words spoken in the audio, "
        "with natural punctuation. Do not add labels, comments, placeholders, or explanations."
    )
    transcribe_prompt: str = (
        "Transcribe the following audio for a web developer. Preserve technical terms, "
        "framework/library names, APIs, acronyms, and code-like words when clear. "
    ...
    def from_env(cls) -> Config:
        ...
        return cls(
            ...
            model_alias=os.environ.get("VOICE2TEXT_MODEL", "google/gemini-3-flash-preview"),
            hotkey_name=os.environ.get("VOICE2TEXT_HOTKEY", "f13").lower(),
        )
...
class StatusPrinter:
    ...
    def __init__(self, hotkey_name: str) -> None:
        self._hotkey = hotkey_name

    def show_recording(self) -> None:
        """Show recording started."""
        print(f"🎙️ RECORDING... (press {self._hotkey.upper()} to stop)")
        notify("Voice2Text", "🔴 🔴 🔴 Recording started")

    def show_transcribing(self) -> None:
        """Show transcribing status."""
        print("⏳ Transcribing...")

    def show_recording_timeout(self, max_duration_sec: float) -> None:
        """Show recording timeout status."""
        minutes = max_duration_sec / 60
        print(f"⏱️ Recording exceeded {minutes:.0f}m, discarded")
        notify("Voice2Text", f"⏱️ Recording discarded after {minutes:.0f} minutes")
...
class Recorder:
    ...
    def _stop_stream(self) -> bool:
        """Stop the active input stream if one exists."""
        stream = self._stream
        if stream is None:
            self._recording = False
            return False
    ...
    def stop(self) -> Path | None:
        ...
        if duration >= self._config.max_duration_sec:
            print(f"  too long ({duration / 60:.1f}m), skipping")
            return None
    ...
    def transcribe(self, audio_path: Path) -> str:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": self._config.transcribe_system_prompt,
                },
                {
                    "role": "user",
                    "content": [
...
class HotkeyHandler:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._hotkey = None if self._insert_enabled else HOTKEY_MAP.get(config.hotkey_name, Key.f13)
...
class App:
    ...
    def __init__(self, config: Config) -> None:
        self._config = config
        self._overlay = StatusPrinter(config.hotkey_name)
    ...
    def _start_recording_timer(self) -> None:
        """Start the auto-discard timer for the current recording."""
        self._stop_recording_timer()
        timer = threading.Timer(self._config.max_duration_sec, self._handle_recording_timeout)
        timer.daemon = True
        self._recording_timer = timer
        timer.start()

    def _stop_recording_timer(self) -> None:
        """Stop the active auto-discard timer, if any."""
        if self._recording_timer is None:
            return
        self._recording_timer.cancel()
        self._recording_timer = None

    def _handle_recording_timeout(self) -> None:
        """Discard an overlong recording without sending it for transcription."""
        with self._lock:
        ...
        self._overlay.show_recording_timeout(self._config.max_duration_sec)
        self._overlay.hide()
...
def retry_transcription(audio_path: Path, config: Config) -> None:
    ...
    try:
        overlay = StatusPrinter(config.hotkey_name)
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  VOICE2TEXT_API_URL         OpenAI-compatible API base URL\n"
            "  VOICE2TEXT_HOTKEY=f13      Hotkey (f1-f13, insert)\n"
            "  VOICE2TEXT_MODEL           Model to use\n"
            "\n"
            "Hotkeys:\n"
            "  F13 (or configured key)    Start/stop recording\n"
            "  Ctrl+C                     Quit"
        ),
